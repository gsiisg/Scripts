...
cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(tf.clip_by_value(y,1e-50,1.0)),
                                              reduction_indices=[1]))
...
for i in range(batch_number):
    ...
    for j in range(batch_size):
        ...
        if index < train_size:
            ...
            image_batch[j] = DownSample4x(cv2.imread('input/train/'+train_class+'/'+train_file,0)/255.0).flatten()
            label_batch[j][int(train_class.split('c')[1])] = 1
    train_step.run(feed_dict={x: image_batch, y_: label_batch})
    print('processed batch %i, last filename %s' % (i,train_filename[-1]))
...
for i in range(test_size):
    ...
    test_image_batch[i] = DownSample4x(cv2.imread('input/train/'+test_class+'/'+test_file,0)/255.0).flatten()
